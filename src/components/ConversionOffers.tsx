...
export default function ConversionOffers({ offers }: ConversionOffersProps) {
    ...
    return (
        <section id="offers" className="py-24 bg-background relative overflow-hidden">
            <div className="absolute inset-0 bg-grid-accent/[0.02] bg-[size:60px_60px] pointer-events-none" />
            <div className="absolute top-0 right-0 w-1/3 h-1/3 bg-accent/5 rounded-full blur-[120px] -z-10" />

            <div className="container mx-auto px-6 relative z-10">
                <div className="max-w-6xl mb-40 border-l-4 border-accent pl-12 relative overflow-hidden text-left">
                    <motion.div
                    ...
                    <h2 className="text-6xl md:text-[11rem] font-bold text-white tracking-tighter mb-16 leading-[0.75] uppercase italic">
                    ...
                    <p className="text-2xl md:text-5xl text-zinc-500 font-medium leading-none tracking-tight max-w-5xl">
                        Specialized conversion systems engineered for <span className="text-white italic underline underline-offset-8 decoration-accent">Pure Performance</span>.
                    </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 xl:gap-8">
                    {displayOffers.map((offer, index) => (
                        <motion.div
                            ...
                            viewport={{ once: true }}
                            className={`group h-full flex flex-col p-10 bg-black border transition-all duration-500 relative cursor-crosshair rounded-3xl overflow-hidden ${offer.highlight
                                ? 'border-accent shadow-2xl shadow-accent/30/20'
                                : 'border-zinc-900 hover:border-zinc-700'
                                }`}
                        >
                            {offer.highlight && (
                                <div className="absolute top-0 right-0 bg-accent text-white px-6 py-2 text-[10px] font-bold uppercase tracking-widest rounded-bl-xl">
                                    BEST VALUE
                                </div>
                            )}

                            <div className={`w-16 h-16 bg-zinc-950 border border-zinc-900 flex items-center justify-center mb-10 transition-all duration-500 group-hover:bg-accent group-hover:text-white rounded-2xl ${offer.color || 'text-zinc-600'}`}>
                                {(() => {
                                    const IconComponent =
                            ...
                            <div className="mb-10 pb-8 border-b border-zinc-900">
                                <span className="text-zinc-700 text-[10px] font-mono font-bold uppercase tracking-widest block mb-2">Starting At</span>
                                <span className={`text-5xl font-bold tracking-tighter italic text-white`}>{offer.price}</span>
                            </div>
                            ...
                            <ul className="space-y-4 mb-12 flex-grow">
                                {offer.features.map((feature, i) => (
                                    <li key={i} className="flex items-center gap-4 text-[11px] font-mono font-bold uppercase tracking-widest text-zinc-800">
                                        <div className="w-2 h-2 bg-zinc-900 group-hover:bg-accent transition-colors rounded-full" />
                                        {feature}
                                    </li>
                                ))}
                            </ul>
                            ...
                            <Link href={offer.link} className="w-full">
                                <Button
                                    className={`w-full h-20 rounded-2xl font-bold text-sm uppercase tracking-[0.4em] transition-all duration-300 ${offer.highlight
                ...
                <div className="mt-20">
                    <div className="p-12 bg-zinc-950 border border-zinc-900 max-w-4xl mx-auto flex flex-col md:flex-row items-center justify-between gap-8 rounded-3xl">
                        ...
                        <Link href="/offers/diagnostic">
                            <Button className="bg-accent hover:bg-accent text-white font-bold text-xs px-8 h-16 rounded-2xl uppercase tracking-[0.4em] transition-all duration-300">
                                Start Revenue Audit
